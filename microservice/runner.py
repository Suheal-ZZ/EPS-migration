...
from .job_registry import ALLOWED_ARGS, SCRIPT_REGISTRY
...
class JobScriptNotFoundError(FileNotFoundError):
    """Raised when a mapped job file cannot be found."""
...
def run_job(job_name: str, options: dict | None = None, timeout_seconds: int = 1200) -> dict:
    """
    Execute a registered migration/validation job in a subprocess.
    ...
    Raises:
        ...
        JobScriptNotFoundError: If the mapped job file is missing.
    ...
    job_path = REPO_ROOT / SCRIPT_REGISTRY[job_name]
    if not job_path.exists():
        raise JobScriptNotFoundError(str(job_path))

    safe_options = options or {}
    command = [sys.executable, str(job_path), *_build_cli_args(safe_options)]
